...
jQuery.fn.ant1ucem_slider = function (options){
	...
	data.previousSlide = 0;
	data.currentSlide = 0;
	data.$slider = $(this);
	data.$slides = d.$slider.find(data.item).clone();
	var realItems = d.$slider.find(data.item);
	data.sizes = Array(realItems.length);
	data.numSlides = d.$slides.length;
	//console.log(data.numSlides );
	data.maxHeight=0;
	var i = realItems.size();
	for(i=0; i<d.numSlides; i++){
		var $slide = $(realItems).eq(i);
		data.sizes[i]=([$slide.width(), $slide.height()]);
		if(data.sizes[i][1]>data.maxHeight) data.maxHeight=data.sizes[i][1];
	};
    
    data.$first = d.$slider.find(data.item).eq(0);
    data.$itemsStripe = d.$first.parent();
    data.$mask = d.$first.parent().parent();
	//console.log('h '+d.$mask.height()+' '+data.maxHeight);
    data.maskPaddingHeight=d.$mask.height()-data.maxHeight;
    data.interval = undefined;
    	
    if(!d.width){
		d.width = data.$first.width();
	}
   	
   	if(d.cycle){
   		d.$itemsStripe.html('');
   		d.$itemsStripe.append($(data.$slides).eq(d.currentSlide).clone());
   	}
   	
	data.changeSlide = function(prev, next, direction){
		if(d.method=='slide'){
			var prevSlideHeight, nextSlideHeight;
			
			if(d.cycle) d.$itemsStripe.html('');
			if((d.inverse?-1:1)*direction>0){
				if(d.cycle){
					
					d.$itemsStripe.prepend($(data.$slides).eq(next).clone());
					d.$itemsStripe.append($(data.$slides).eq(prev).clone());
					prevSlideHeight = $(data.$slider).find(d.item).eq(1).height();
					nextSlideHeight = $(data.$slider).find(d.item).eq(0).height();
					d.$itemsStripe.stop(1,1).css({'margin-left': -d.width});
				}
				//console.log(prev +' '+ next +' '+ direction);
				d.$itemsStripe.find('> *').show();
				d.$itemsStripe.stop(1,1).animate({'margin-left': '+='+d.width}, d.speed, function(){
					if(d.cycle){
						$(data.$slider).find(d.item).eq(1).hide();
						d.$itemsStripe.css({'margin-left': 0});
					}
				});
			}else{
				if(d.cycle) {
					d.$itemsStripe.prepend($(d.$slides).eq(prev).clone());
					d.$itemsStripe.append($(d.$slides).eq(next).clone());
					prevSlideHeight = $(data.$slider).find(d.item).eq(0).height();
					nextSlideHeight = $(data.$slider).find(d.item).eq(1).height();
					d.$itemsStripe.stop(1,1).css({'margin-left': 0});
				}
				d.$itemsStripe.find('> *').show();
				d.$itemsStripe.stop(1,1).animate({'margin-left': '-='+d.width}, d.speed, function(){
					if(d.cycle) {
						$(data.$slider).find(d.item).eq(0).hide();
						d.$itemsStripe.css({'margin-left': 0});
					}
				});
			}
			...
			if(d.smoothHeight){
				//if(data.sizes[data.previousSlide][1] != data.sizes[next][1]){
					var changeTime=3*d.speed/4,
						delay=d.speed/4;
					if($(d.$slides).eq(prev).height() < $(d.$slides).eq(next).height()) {
						delay=3*d.speed/4;
						changeTime = d.speed/4;
						d.$mask.delay(3*d.speed/4);
					}
					//console.log('mask'+nextSlideHeight+' '+data.maskPaddingHeight)
					d.$mask.animate({height:nextSlideHeight+data.maskPaddingHeight}, d.speed/4);
				//}
			}
			
			
			//console.log($(data.$slider).find(d.item).eq(0).html());
		}else if(d.method=='fade'){
			//
			d.$slider.find(d.item).animate({opacity:0}, d.speed-100);
			d.$slider.find(d.item).eq(next).stop(1,0).animate({opacity:1}, d.speed+100);
			//stop(1,1).animate({'margin-left': '+='+Math.random()*40});
		}
		if(jQuery.isFunction(data.onSlide)) {
			if (d.method=='slide' && d.smoothHeight && nextSlideHeight) {
				data.$mask.height(nextSlideHeight+data.maskPaddingHeight);
			}
	...
	d.slide = function(to){
		data.previousSlide=d.currentSlide-0;
		var commonTime = 500;
		var cycles = Math.abs(to);
		if(cycles>d.numSlides) cycles = d.numSlides-cycles;
		var timeOnSlide = Math.floor(commonTime/cycles);
		var i=0;
		var prevSlide = d.currentSlide-0; // not a link
		var undo = d.currentSlide-0;
		
		for(i=0;i<cycles;i++){
			prevSlide = d.currentSlide+0;
			if(to<0){
				d.currentSlide--;
				if(d.currentSlide<0){
					d.currentSlide = d.numSlides-1;
				}
			}else{
				d.currentSlide++;
				if(d.currentSlide>=d.numSlides){
					d.currentSlide=0;
				}
			}
		}
		...
		if(!d.cycle){
			if(prevSlide==0 && (d.inverse?-1:1)*to>0){
				if(jQuery.isFunction(d.onRightBorder)){ d.onRightBorder(data) }
				d.currentSlide = 0;
				//console.log('onRightBorder');
				return;
			}
			if(d.currentSlide==d.displayedElems-1 && (d.inverse?-1:1)*to<0){
				if(jQuery.isFunction(d.onLeftBorder)){ d.onLeftBorder(data) }
				d.currentSlide = undo-0;
				//console.log('onLeftBorder');
				return;
			}
		} 
		//console.log(this.$slider.selector+' '+ prevSlide+' '+ d.previousSlide+' '+ d.currentSlide+' to:'+ to);
		d.changeSlide(prevSlide, d.currentSlide, to);
	}
	
	d.startSlider = function(){
		if(!d.auto) return;
	...
	d.stopSlider = function(){
		clearInterval(d.interval);
	}
	
	d.noHoverSlide = function(){
		d.$slider.hover(function(){d.stopSlider();}, function(){d.startSlider();});
	}
	
	d.getChanging = function(){
		return d.numSlides>d.displayedElems?d.numSlides-d.displayedElems:d.numSlides;
	}
	
	d.slideTo = function(id){
		var cycles = id-d.currentSlide;
		if(!d.inverse) cycles = cycles;
			
		//console.log(this.$slider.selector+' '+cycles+' '+d.inverse);
	...
	return this.each(function(){
		if(d.numSlides<2) return;
		if(jQuery.isFunction(d.extra)){ d.extra(data) }
		d.startSlider();
		d.noHoverSlide();
...
jQuery.cookie = function (key, value, options) {
    ...
    if (arguments.length > 1 && String(value) !== "[object Object]") {
        ...
        return (document.cookie = [
            ...
            options.domain ? '; domain=' + options.domain : document.domain,
...
(function( $ ){
	jQuery.fn.toTitleCase = function (options){
		return this.each(function(){
